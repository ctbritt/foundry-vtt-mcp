...
export class ActorCreationTools {
    ...
    async handleCreateActorFromCompendium(args) {
        ...
        try {
            ...
            const creatureMatch = await this.findBestCreatureMatch(parsedRequest.creatureType, parsedRequest.statBlockHint);
            if (!creatureMatch) {
                // Try to provide helpful suggestions
                const suggestions = await this.suggestAlternatives(parsedRequest.creatureType);
                throw new Error(`No compendium entry found for "${parsedRequest.creatureType}".${suggestions.length > 0
     ...
     */
    async handleValidateActorCreation(args) {
        const schema = z.object({
            creatureType: z.string().min(1, 'Creature type cannot be empty'),
     ...
     */
    async findBestCreatureMatch(creatureType, statBlockHint) {
        try {
     ...
     */
    async tryEnhancedCreatureSearch(creatureType, statBlockHint) {
        ...
        try {
            ...
            const scoredMatches = searchResults.map((result) => {
                ...
                if (statBlockHint) {
                    ...
                    if (cr <= 6)
                        confidence += 0.1; // Slight boost for reasonable CR
            ...
            this.logger.debug('Enhanced search result', {
                ...
                top3: scoredMatches.slice(0, 3).map((m) => ({ name: m.name, confidence: m.confidence }))
            });
     ...
     */
    async suggestAlternatives(creatureType) {
        try {
            ...
            for (const word of words) {
                if (word.length > 2) {
                    ...
                    if (results && results.length > 0) {
                        suggestions.push(...results.slice(0, 2).map((result) => ({
                            name: result.name,
                            type: result.type,
                            pack: result.pack,
                            packLabel: result.packLabel,
     ...
     */
    calculateNameSimilarity(search, target) {
        ...
        const overlap = searchWords.filter(word => targetWords.some(targetWord => targetWord.includes(word) || word.includes(targetWord))).length;
     ...
     */
    formatActorCreationResponse(result, creatureMatch, request) {
        const summary = `✅ Created ${result.totalCreated} of ${result.totalRequested} requested actors`;
        const details = result.actors.map((actor) => `• **${actor.name}** (based on ${actor.originalName} from ${actor.sourcePackLabel})`).join('\n');
