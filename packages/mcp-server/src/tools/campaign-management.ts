...
export class CampaignManagementTools {
   ...
   */
  async handleUpdateCampaignProgress(args: any): Promise<any> {
    try {
      const requestSchema = z.object({
      ...
      if (request.newStatus === 'completed') {
        ...
        if (part) {
          if (request.subPartId) {
            ...
            if (subPart) subPart.completedAt = Date.now();
          } else {
   ...
   */
  private generatePartHTML(part: CampaignPart, partNumber: number, campaign: CampaignStructure): string {
   ...
   */
  private updatePartStatus(campaign: CampaignStructure, partId: string, newStatus: string, subPartId?: string): boolean {
    ...
    if (subPartId) {
      ...
      subPart.status = newStatus as any;
    } else {
      part.status = newStatus as any;
    }
   ...
   * Find part by ID
   */
  private findPart(campaign: CampaignStructure, partId: string): CampaignPart | null {
   ...
   */
  private async loadCampaignStructure(campaignId: string): Promise<CampaignStructure | null> {
